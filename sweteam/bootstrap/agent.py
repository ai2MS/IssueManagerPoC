"""Core Agent Class code sfor OpenAI"""
import os
import json
import sys
import time
...
class OpenAI_Agent:
    ...
    def write_binary_file(self, filename: str, content: str, force: bool = False) -> str:
        """Write binary info to a file
        ...
        Example::
            >>> agent = OpenAI_Agent("tester")
            >>> print(agent.write_binary_file("deleteme.test", "This is a test, please delete me.", force=True))
            File deleteme.test has been written successfully.
        """
        logger.debug(f"<{self.name}> - write_binary_file {filename} - {content}")
        if not os.path.exists(filename) or force:
            try:
                ...
                logger.info(f"<{self.name}> - write_binary_file {filename} successfully.")
                return f"File {filename} has been written successfully."
            except Exception as e:
                logger.error(f"<{self.name}> - write_binary_file Failed to write to file {filename}, received Error {e}.")
                return f"Error: {str(e)} ____ {e}"
        else:
            result = json.loads(self.read_from_file(filename))
            result['rejection'] = f"Binary file {filename} already exists, its content is in this message, please make sure the the new content is not causing existing code to fail, then use `force=True` to overwrite the existing content."
            return json.dumps(result)
        
    def update_text_file(self, filename: str, ed_script: str, return_content: bool = True) -> str:
        ...
        Example::
            >>> agent = OpenAI_Agent("tester")
            >>> print(agent.update_text_file("deleteme.test", "0a\\nThis is a test, please delete me.\\n.\\n"))
            File deleteme.test has been updated successfully.
        """
        import re
        logger.debug(f"<{self.name}> - update_text_file {filename} - using script {ed_script}")
        if os.path.exists(filename):
            logger.debug(f"<{self.name}> - update_text_file file {filename} already exists, reading in to be changed")
            with open(filename, "r", encoding="utf-8") as f:
                content = f.readlines()
        else:
            logger.debug(f"<{self.name}> - update_text_file file {filename} does not exist, creating new")
            content = []
        ...
        for script_line in ed_script.split("\n"):
            if expect_line_cmd_spec:
                ...
                if match:
                    ...
                    start_line = int(match.group(1).replace('$', old_content_lines) if match.group(1) else 1) - (command != 'a')
                    end_line = int(match.group(2).replace('$', old_content_lines)) if match.group(2) else start_line + (command != 'a')
                    new_lines = []
                    if command == 'd':
                    ...
                    logger.error(f"<{self.name}> - update_text_file wrongly formed ed_script ")
                    return f"update_text_file: wrongly formed ed_script command: {script_line}. Only accept proper ed script in the format generated by diff -e. for example ``37,$d\n`, `25,31c\nchange to new line\n.\n`, or 0a\nAdd a line\n.\n`. Later change blocks should be provided first so that the line specifiers are not affected by the changes."
            else:
        ...
        try:
            ...
            logger.info(f"<{self.name}> - update_text_file {filename} successfully.")
        except Exception as e:
            logger.error(f"<{self.name}> - update_text_file Failed to write to file {filename}, received Error {e}.")
            return f"Error: {str(e)} ____ {e}"
        result = {"filename": filename, "status": "update successful", "new content": content}
        return json.dumps(result)
        ...
        while self.run.status in ["queued", "in_progress", "requires_action"]:
            ...
            if self.run.status == "requires_action":
                try:
                    ...
                    if tools_output:
                        ...
                        logger.debug(f"<{self.name}> TASK:STEPs -Function(s) returned {str(tools_output_head)}")
            ...
            if self.run.status in ["expiried", "failed"]:
                ...
                if retry_count > 0:
                    ...
                    input("thread.run status failed, please check:....")                    
                    if (last_error :=getattr(self.run,'last_error', {})) and (error_code := getattr(last_error, 'code', {})):
                        logger.warning(f"<{self.name}> TASK: thread.run returned error {error_code}")
